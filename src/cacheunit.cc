...
void CacheUnit::push(int port, Packet *p)
{
    ...
    if(port == 0)//this is a probing message
    {
        ...
        for (int i = 0; i < (int) numberOfIDs; i++) {//get the ID
    ...
    else if(port == 1)
    {
        ...
        if(testFID == gc->iLID)
        {
            ...
            memcpy(&numberOfIDs, p->data()+14+FID_LEN, sizeof(numberOfIDs)) ;//# of IDs
            for (int i = 0; i < (int) numberOfIDs; i++) {//get the full ID
                IDLength = *(p->data()+14+FID_LEN+sizeof(numberOfIDs)+index);
                IDs.push_back(String((const char *) (p->data()+14+FID_LEN+sizeof(numberOfIDs)+sizeof(IDLength)+index),\
                                    IDLength * PURSUIT_ID_LEN));
                index = index + sizeof (IDLength) + IDLength * PURSUIT_ID_LEN;
            ...
            {//check local cache
                if((*cache_iter)->matchIID(IDs))
                {//local cache found, push back to subscriber
                    cachefound = true ;
                    int reverse_proto ;
                    int prototype ;
                    cp_integer(String("0x080d"), 16, &reverse_proto);//the data packet ethernet type
    ...
    {//this is data packet, check local cache to see if this data has been cached before
        Vector<String> IIDs ;
        int i = 0 ;
        bool cachefound = false ;
        char* data ;
        ...
        {//skip if this data is controll message
            output(3).push(p) ;
        }
        else
        {
            ...
            case SCOPE_PROBING_MESSAGE:
            {
                ...
                {//check local cache
                    ...
                    {//if local cache found
                        ...
                        {//if information item found add to Bloom Filter
                            BFforIID.add2bf(*iter) ;
                        }
                        //update total distance and # of cache
                ...
                for(cache_iter = cache.begin() ; cache_iter != cache.end() ; cache_iter++)
                {
                    ...
                    {//local cache found
                        ...
                        for(Vector<String>::iterator iter = (*cache_iter)->IIDs.begin() ; iter != (*cache_iter)->IIDs.end() ; iter++)
                        {
                            ...
                            {//check if the information item has been satisfied before
                                ebf.add2bf(*iter) ;
                        ...
                        else
                        {
                            ...
                            if(ebf != ibf)
                            {//check if the request has been satisfied yet
                                WritablePacket* packet = p->uniqueify() ;
...
void CacheUnit::sendbackData(Vector<String>& SIDs, Vector<String>& IIDs, BABitvector FID, CacheEntry* ce)
{
    int reverse_proto ;
    int prototype ;
    cp_integer(String("0x080d"), 16, &reverse_proto);//data packet ethernet type
    prototype = htons(reverse_proto);
    Vector<String> IDs ;
