                                 ...
                                 HashTable<String, unsigned int> each_sub_hopcount)
{
    ...
    for(str_map_iter = FID_to_each_sub.begin() ; str_map_iter != FID_to_each_sub.end() ; str_map_iter++)
    {//send probing message to each subscriber
...
void LocalProxy::handleScopeProbingMessage(Vector<String> IDs, Packet* p)
{
    ...
    if(noofcache > 0)//if there're any cache along the path
    ...
    for(Vector<String>::iterator iter = IDs.begin() ; iter != IDs.end() ; iter++)
    {
        ...
        {//if the subscriber have not received INFO_PUBLISHED message
        ...
        {//for each information items under the scope
            bool cached = false ;
            ...
            {//if the information item has not been updated before
                as->iid_FID_map.set(iid_iter->_strData, to_pub_FID) ;
                as->iid_p2sFID.set(iid_iter->_strData, to_sub_FID) ;
                if(cached)//if the item has been cached along the path, save the avg_hop_count
                    as->iid_distance_map.set(iid_iter->_strData, avg_hop_count) ;
                else//if not cached, save the hop count from the publisher
        ...
        {//if all the probing message are received
            subreq_sent = true ;//subrequest has already been sent, no need to send again
            unsigned char type = SUB_SCOPE_MESSAGE ;
	/*note that Bloom Filter and Bitvector can not be put in hashtable, must have a hash function or sth, so use String*/
            HashTable<String, BloomFilter> strfid_ibf ;/*map fid to information bloom filter, fid is in string form;
            						this variable indicates how to retrieve information*/
            HashTable<String, BABitvector> str_fid ;//a hashtable map string to fid, the key value is the String form
            					    //of the fid
            HashTable<String, BABitvector> str_p2sfid ;
            for(StringSetIter iid_iter = as->IIDs.begin() ; iid_iter != as->IIDs.end() ; iid_iter++)
            {//kanycast determine how to retreive the content
                ...
                BABitvector tempp2sfid(FID_LEN*8) ;
                String tempfidstr ;

                tempp2sfid = as->iid_p2sFID.get(iid_iter->_strData) ;
                tempfid = as->iid_FID_map.get(iid_iter->_strData) ;//get the fid
                tempfidstr = String((const char*)(tempfid._data), FID_LEN) ;//turn fid into string
                str_fid.set(tempfidstr, tempfid) ;
                str_p2sfid.set(tempfidstr, tempp2sfid) ;//key is the string form of s2p fid, the value is p2sfid
                tempibf = strfid_ibf.get(tempfidstr) ;//get the information bloom filter
		...
		memcpy(packet->data(), str_fid[strfid_ibf_iter->first]._data, FID_LEN) ;
                ...
                memcpy(packet->data()+FID_LEN+sizeof(type)+sizeof(numberOfIDs)+index+EBFSIZE+IBFSIZE, str_p2sfid[strfid_ibf_iter->first]._data, FID_LEN) ;
...
/*tell the publisher to send data to subscriber*/
